...
namespace GT
{
	...
	void ScriptEngine::Init()
	{
		...
		s_Data->EntityClass = ScriptClass("GT", "Main");
	}
	...
	void ScriptEngine::LoadAssembly(const std::filesystem::path& filepath)
	{
		...
		s_Data->AppDomain = mono_domain_create_appdomain("GTScriptRuntime", nullptr);
	...
	void ScriptEngine::OnUpdateEntity(Entity entity, Timestep ts)
	{
		...
		GT_CORE_ASSERT(s_Data->EntityInstances.find(entityUUID) != s_Data->EntityInstances.end(),"");
	...
	void ScriptEngine::LoadAssemblyClasses(MonoAssembly* assembly)
	{
		...
		const MonoTableInfo* typeDefinitionsTable = mono_image_get_table_info(image, MONO_TABLE_TYPEDEF);
		int32_t numTypes = mono_table_info_get_rows(typeDefinitionsTable);
		MonoClass* entityClass = mono_class_from_name(image, "GT", "Entity");
