	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					GT_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
...
#define GT_FUNC_SIG __func__
#else
#define GT_FUNC_SIG "GT_FUNC_SIG unknown!"
#endif

#define GT_PROFILE_BEGIN_SESSION(name, filepath) ::GT::Instrumentor::Get().BeginSession(name, filepath)
#define GT_PROFILE_END_SESSION() ::GT::Instrumentor::Get().EndSession()
#define GT_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::GT::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
											   ::GT::InstrumentationTimer timer##line(fixedName##line.Data)
